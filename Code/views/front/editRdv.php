...
<a href="index1.php"><img src="images/logo1.png"></a>				</div>
  ...
  <div class="">
	...
	<input type="hidden" name="Id" class="form-control"  placeholder="yyyy-mm-dd" value="<?PHP echo $Id ?>">
     <label for="inputEmail4">Date</label>
      <input type="date" name="dateR" class="form-control"  placeholder="yyyy-mm-dd" value="<?PHP echo $date ?>">
    </div>
    <div class="col mb-3">
     <label for="inputEmail5">Time</label>
      <input type="time" name="timeR" class="form-control"   placeholder="example 12h" value="<?PHP echo $time ?>">
    </div>
    <div class="col mb-3">
     <label for="inputEmail5">Product Reference</label>
      <select name="refp" class="custom-select my-1 mr-sm-2">
<?php echo getnames();?>
</select>
    </div>
  </div>
	...
	if (isset($_POST['edit']))
{
	...
	else if (strtotime($_POST['dateR']) > strtotime('now'))
{
	 $ec= new Rdv($_POST['dateR'],$_POST['timeR'],$_POST['refp']);
        $e= new RdvC();
		 $time=$e->rechercherProduit($_POST['refp'],$_POST['dateR'],$_POST['timeR']) ;
	 
	 if($time>=1)
	 {
		  echo "<script type='text/javascript'>";
       echo "alert('This product has already been reserved on that time!!');
             window.location.href='dispalyRdvFront.php';";
        echo "</script>";
	 }
	 else
	 {
