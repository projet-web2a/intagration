...
{
	$mar=$_GET['mar'];
	$list_produit=$avis->list_produit($mar);

}
if (isset($_GET['refe']))
{
	$refe=$_GET['refe'];
	$list_produit1=$avis->list_produit_refe($refe);
...
<body>
	...
	<section class="banner-bottom-wthreelayouts py-lg-5 py-3">
		<div class="container">
			<h3 class="tittle-w3layouts text-center my-lg-4 my-4">Avis</h3>
			<div class="inner_sec">
				<p class="sub text-center mb-lg-5 mb-3">Votre avis nous intéresse :**</p>
				...
				<div class="contact_grid_right">
					<form method="Post">
					<div class="form-group" >
					<label for="formGroupExampleInput"> <strong>Marque Produit </strong> </label>
					</div>
					<div class="form-group">
					<div class="dropdown">
  <a class="btn btn-light  btn-lg btn-block dropdown-toggle" href="#" role="button" id="dropdownMenuLink" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
    Choisir une marque
  </a>

  <div class="dropdown-menu btn-block" aria-labelledby="dropdownMenuLink">
  	 <?php foreach($listeproduit as $produit): ?>
    <a class="dropdown-item" href="avis.php?mar=<?=$produit->mar?>"><?= $produit->mar; ?></a>
               			<?php endforeach; ?>
  </div>
</div>
					</div>
				</form>
				<?php if (isset($_GET['mar']) )  {   ?>

				  	 <?php// foreach($list_produit as $produit): ?>
					<div class="form-group">
					<div class="dropdown">
  <a class="btn btn-light  btn-lg btn-block dropdown-toggle" href="#" role="button" id="dropdownMenuLink" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
    Choisir une reference
  </a>

  <div class="dropdown-menu btn-block" aria-labelledby="dropdownMenuLink">
  	 <?php foreach($list_produit as $produit): ?>
    <a class="dropdown-item" href="avis.php?mar=<?=$produit->mar?>&refe=<?= $produit->refe; ?>"><?= $produit->refe; ?></a>
               			<?php endforeach; ?>
  </div>
</div>
					</div>
									<?php if (isset($_GET['refe']) )  {   ?>


				  	 <?php foreach($list_produit1 as $produit): ?>

				<form action="ajouter_avis.php?refe=<?=$produit->refe?>&marque=<?=$produit->mar?>" method="post" name="f" onsubmit="return verifa()">
	 ...
	 <div class="form-group">
							...
							<img src="images/<?=$produit->url?>" class="img-fluid" alt="" width="300" height="300">
							</div>
<div class="stars">
